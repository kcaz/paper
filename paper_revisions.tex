...
The function $p(x)$ controls the shape of the penalty function, while scalar $\lambda_S$ controls the overall scaling of the penalization of differences between fused coefficients. $\lambda_S$ controls the tradeoff between fitting the expression data-sets individually and producing a set of networks that conform to evolutionary prior knowledge. 
...
[KARI - INSERT HOW GOLD STANDARD WAS OBTAINED]
...
For real data, we examined recovery of a known gold standard in \textit{Bacillus subtilis} \cite{michna_subtiwikidatabase_2014}. 
...
Although we developed the approach initially with cross-species network inference in mind, the core algorithm -- an implementation of L2-fused multi-task regression with arbitrary constraint graphs -- is flexible enough to represent other useful constraint structures. Any biological prior representing information about expected regulatory similarity can be utilized, even if the prior provides no information about the magnitude or direction of regulation. 
We demonstrate this flexibility through the incorporation of operon structure into the gene-regulatory network inference problem, and show an improvement in performance in bacterial data when combined with a prior consisting of some known interactions (Figure \ref{fusedl2-real}). In this application, fusion reflects the assumption that genes in the same operons have similar regulators \cite{lawrence_shared_2002}. 


Our work is premised on the assumption that distinct organisms share many conserved regulatory interactions, so that networks obtained in related species should similar to one another in a manner predictable by the evolutionary relationships of the organisms' genes. However, we know that orthology is an imperfect predictor of functional similarity\cite{gabaldon_functional_2013, studer_how_2009, nehrt_testing_2011}. Moreover, it can be critically important to understand how systems differ from one another. Existing approaches to the genome-wide testing of conserved function learn regulatory networks separately, then compare to identify conservation \cite{aytes_cross-species_2014}. 
Because network inference is typically underconstrained, fitting a network that describes a particular set of experimental observations amounts to sampling a single network from a large set of networks that fit the data equally (or almost equally) as well. As a result, the existence of a difference between corresponding regulatory interactions in a pair of independently fit networks is weak evidence that a difference truly does exist; some other networks, which do exhibit conservation, may have fit the data almost as well. Uncoupled global network inference algorithms are a very weak tool for uncovering evolutionary divergence. Our method is biased to recover networks in which conservation does hold; a failure to find conservation is therefore much much more direct evidence that the networks have truly diverged. 
