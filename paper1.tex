...
We also implement MCP [IS THIS MODIFIED TO BE L2 like at the origin?]
...
As in [Fan Li], we use solve using iterative local quadratic approximation. Specifically,

\begin{equation}
\begin{array}{l}
\beta^S(t)\text{ is the network on iteration } t
\\
...
\\
\beta^S(t+1) \text{ is obtained by fitting the ridge-fused model with fusion constraints given by the above }\lambda s
\end{array}
\end{equation}
