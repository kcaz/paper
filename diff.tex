...
\DIFadd{Understanding which interactions are functionally dissimilar is of direct biological interest, but existing approaches to network inference are unable to effectively rule out conservation. Observing a large difference in the weights of regulatory interactions obtained though independent inference of multiple networks is the best evidence against conservation available with existing network inference techniques. This evidence is very weak, however}\DIFaddend ; because network inference is typically underconstrainted \cite{marbach_revealing_2010-1}, there could be \DIFdelbegin \DIFdel{other plausible networks in which the coefficients being compared are similar}\DIFdelend \DIFaddbegin \DIFadd{a different set of networks for which conservation does hold, and which fit the data almost as well}\DIFaddend . Our adaptive fusion method allows us to learn differences in networks predicted to be similar based on orthology or gene matching: because the constraints we impose favor weight configurations in which fused interaction weights are similar to one another, a solution in which a priori similar regulatory interactions are found to be different is strong evidence that they really are different. 
\DIFdelbegin \DIFdel{When differences in regulatory interactions arise where they are predicted to be similar, the network inference procedure would be improved by relaxing or ignoring these evolutionary constraints. However, we don't know a priori which constraints to enforce and which to relax. 
}\DIFdelend 
...
\section{Methods}
\DIFaddbegin \DIFadd{Although our approach is generalizable to a wide variety of multi-source network inference problems, we begin with the concrete example of network inference in two related species. Our approach to multi-species network inference is based on the hypothesis that gene regulation in related species is governed by similar but not necessarily identical gene regulatory networks, due to conservation of function through evolution. We represent conservation of network function by introducing constraints into the objective function for network inference that penalize differences between the weights of regulatory interactions believed to be a priori similar. These constraints favor the generation of similar networks for related species, and in the generally under-constrained regime of network inference can improve the accuracy of network recovery. We then go on to introduce a method to test the assumption of conserved network structure, and to relax the associated constraints on pairs of interactions for which data does not support conservation.
}

\DIFaddend \subsection{Gene regulatory network}
We model \DIFdelbegin \DIFdel{each gene's expression as a linear combination of transcription factors (TFs) , and we }\DIFdelend \DIFaddbegin \DIFadd{the transcription rate of each gene as a weighted sum of transcription factor (TF) expression, and }\DIFaddend seek to identify the \DIFdelbegin \DIFdel{TFs which regulate each gene, and the strength of the TF-gene interactions. In the }\DIFdelend \DIFaddbegin \DIFadd{identities and regulatory weights of these TFs. This formulation matches that of the existing }\DIFaddend Inferelator algorithm, \DIFaddbegin \DIFadd{which models gene expression with }\DIFaddend linear differential equations \DIFdelbegin \DIFdel{are used to model changes in gene expression }\DIFdelend \cite{bonneau_inferelator:_2006-1}\DIFaddbegin \DIFadd{. }\DIFaddend Our primary data for learning gene regulatory networks is expression data, \DIFaddbegin \DIFadd{consisting of }\DIFaddend time-series and steady state experiments. We use time-series data to approximate the rate of change of expression, and \DIFdelbegin \DIFdel{we }\DIFdelend treat steady-state experiments as \DIFdelbegin \DIFdel{conditions that have reached equilibrium }\DIFdelend \DIFaddbegin \DIFadd{having reached equilibrium (constant expression)}\DIFaddend . The rate at which $x_{i}$, the observed mRNA expression of gene $i$, changes, is governed by degradation of existing transcripts with rate $\alpha$ plus a linear combination of transcription factor (TF) expressions. 
\begin{equation}
...
\end{equation}
\DIFaddbegin \DIFadd{for time series
}\begin{equation}
\DIFadd{0 = \sum \beta_{i,j}x_{j}(t_k)
}\end{equation}
\DIFadd{for steady state
...
\noindent \DIFdelbegin \DIFdel{We approach the learning of the $\beta$ matrix using }\DIFdelend \DIFaddbegin \DIFadd{The matrix $\beta$ can be solved using linear }\DIFaddend regression. Because there are typically far fewer conditions than possible regressors (TFs), we introduce a ridge regularization constraint with weight $\lambda_R$ and solve
\begin{equation}
\argmin_\beta\vert \vert X\beta - Y_2 \vert \vert ^2 + \lambda_R \vert \vert \beta \vert \vert ^2
\end{equation}
This is similar to the formulation used in the Inferelator algorithm, which we extend to the case of simultaneously inferring \DIFdelbegin \DIFdel{the GRNs of multiple species}\DIFdelend \DIFaddbegin \DIFadd{multiple networks}\DIFaddend . 

\subsection{Fused gene regulatory networks}
\DIFdelbegin \DIFdel{We hypothesize that related species should be governed by similar but not necessarily identical gene regulatory networks. We approach the problem of cross-species GRN inference by introducing }\DIFdelend \DIFaddbegin 

\DIFadd{Information about the conserved structure of gene regulation is introduced through the incorporation of }\DIFaddend constraints into the above regression formulation\DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{. These constraints }\DIFaddend penalize differences between interaction weights \DIFaddbegin \DIFadd{in the networks of multiple species }\DIFaddend that are expected to be similar based on \DIFdelbegin \DIFdel{, say, gene orthology}\DIFdelend \DIFaddbegin \DIFadd{prior biological knowledge}\DIFaddend . We can then solve the penalized regression problems simultaneously, in order to obtain a GRN for each species. Consider the case of organisms $A$ and $B$, governed by GRNs $\beta^A$ and $\beta^B$ (the following approach applies equally well to more than two species but for simplicity we continue with the case of two species). If TF $g^A$ in organism $A$ and TF $h^B$ in organism $B$ are orthologs, and gene $k^A$ and $l^B$ are orthologs, then we expect that the $g^A \rightarrow k^A$ interaction weight should be similar to the $h^B \rightarrow l^B$ interaction weight, and we introduce a fusion constraint between these analogous interactions. In terms of the above regression formulation, we expect that $\beta^A_{g,k} \approx \beta^B_{h,l}$, and include a penalty term $p_{\lambda_S}(\beta^A_{g,k} - \beta^B_{h,l})$ \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{in }\DIFaddend the quantity being minimized in order to \DIFdelbegin \DIFdel{ensure }\DIFdelend \DIFaddbegin \DIFadd{encourage }\DIFaddend similarity. The \DIFaddbegin \DIFadd{function $p_{\lambda_S}(x)$ controls the shape of the relationship between weight dissimilarity and penalty, while }\DIFaddend parameter $\lambda_S$ controls \DIFdelbegin \DIFdel{how highly to penalize }\DIFdelend \DIFaddbegin \DIFadd{the overall scaling of the penalization of }\DIFaddend differences between fused coefficients\DIFdelbegin \DIFdel{and }\DIFdelend \DIFaddbegin \DIFadd{. $\lambda_S$ }\DIFaddend controls the tradeoff between fitting the expression data \DIFaddbegin \DIFadd{alone }\DIFaddend and producing a set of networks that conform to evolutionary \DIFdelbegin \DIFdel{constraints}\DIFdelend \DIFaddbegin \DIFadd{prior knowledge}\DIFaddend . This gives us the final equation to be minimized 
\begin{equation}
...
\argmin_{\DIFdelbegin \DIFdel{(\beta^A, }\DIFdelend \beta\DIFdelbegin \DIFdel{^B)}\DIFdelend \DIFaddbegin \DIFadd{^S}\DIFaddend } \displaystyle\sum_{S \in \DIFdelbegin \DIFdel{(A, B)}\DIFdelend \DIFaddbegin \DIFadd{\text{species}}\DIFaddend } \vert \vert X^S\beta^S - Y^S \vert \vert ^2 + \lambda_R \vert \vert \beta^S \vert \vert ^2 + \displaystyle \sum_{\substack{(g,h) \in orth,\\
 (h,l) \in orth}}x p_{\lambda_S}(\beta^{S_1}_{g,k} - \beta^{S_2}_{h,l})
\end{equation}

where the second sum is over pairs of interactions with fusion constraints. \DIFdelbegin %DIFDELCMD < 

%DIFDELCMD < %%%
\subsection{\DIFdel{Fused L2 penalty for multi-source network inference}}
%DIFAUXCMD
\addtocounter{subsection}{-1}%DIFAUXCMD
\DIFdel{When the penalty $p_{\lambda_S}$ is }\DIFdelend \DIFaddbegin \DIFadd{In the fused L2 algorithm we develop, the penalty function is equal to }\DIFaddend the L2 norm \DIFdelbegin \DIFdel{, as in our fused L2 approach:
}\DIFdelend \DIFaddbegin \DIFadd{of the difference in regulatory weight of fused coefficients. That is, $p_{\lambda_S}(x)=x^2$. The overall objective function for network inference is then:
}

\DIFaddend \begin{equation}
\argmin_{\DIFdelbegin \DIFdel{(\beta^A, }\DIFdelend \beta\DIFdelbegin \DIFdel{^B)}\DIFdelend \DIFaddbegin \DIFadd{^S}\DIFaddend } \displaystyle\sum_{S \in \DIFdelbegin \DIFdel{(A, B)}\DIFdelend \DIFaddbegin \DIFadd{\text{species}}\DIFaddend } \vert \vert X^S\beta^S - Y^S \vert \vert ^2 + \lambda_R \vert \vert \beta^S \vert \vert ^2 + \displaystyle \lambda_S \sum_{\substack{(g,h) \in orth,\\
 (h,l) \in orth}} \vert \vert (\beta^{S_1}_{g,k} - \beta^{S_2}_{h,l}) \vert \vert ^2
\end{equation}

\DIFaddbegin \DIFadd{Because every component of the objective function is an L2 norm, }\DIFaddend the entire problem \DIFaddbegin \DIFadd{is convex, and }\DIFaddend can be solved through linear regression with a suitable design matrix. \DIFdelbegin \DIFdel{The regulators of a pair of genes can be solved independently as long as the pair is not linked by fusion constraints, or a chain of fusion constraints; thus the problem of cross-species GRN inference decomposes into a large number of manageable regression problems as long as the orthology mappings are sparse.
}\DIFdelend %DIF > The regulators of a pair of genes can be solved independently as long as the pair is not linked by fusion constraints, or a chain of fusion constraints; thus the problem of cross-species GRN inference decomposes into a large number of manageable regression problems as long as the orthology mappings are sparse.

In order to better understand \DIFdelbegin \DIFdel{this}\DIFdelend \DIFaddbegin \DIFadd{the behavior of this objective function}\DIFaddend , consider the case where there is a one-to-one orthology between the species being considered (ie different cell-lines of the same organism). The choice of $\lambda_S$ allows one to interpolate between fitting each network independently ($\lambda_S=0$) and pooling data together as if it came from one source ($\lambda_S=\inf$). Part of the appeal of the approach, however, is that it allows pooling of data even when there is incomplete orthology. By introducing constraints on the similarity of individual interactions, rather than on the networks as a whole \cite{liu2011temporal}, we can pool some information across species even when an arbitrarily small fraction of genes have orthologs. This is particularly useful when dealing with a large number of species; pairwise orthologies may be nearly complete even when the number of genes present in every organism is small. 


\subsection{Solving fused L2 problems using augmented matrices}
We \DIFaddbegin \DIFadd{begin with the problem of constructing a design matrix to solve a fused L2 regression problem with a single response variable, because problems with multiple response variables and with multiple data sources can be converted into this form through vectorization. We then go on to show that, although the vectorized solution involves solving an impractically large system of equations, under typical biological conditions the structure of constraints allow the problem to be broken up into many smaller subproblems. 
}

\DIFadd{We }\DIFaddend use the fact that ridge \DIFdelbegin \DIFdel{regression can be solved by ordinary least squares regression on an augmented dataset, }\DIFdelend \DIFaddbegin \DIFadd{constraints can be incorporated into a least-squares regression problem }\DIFaddend by appending a scaled identity matrix to the design matrix, and \DIFaddbegin \DIFadd{a corresponding number of }\DIFaddend zeros to the response \DIFdelbegin \DIFdel{matrix. The problem of solving a multi-valued regression problem can also be converted into the problem of solving a single valued }\DIFdelend \DIFaddbegin \DIFadd{vector. Similarly, a fusion constraint $\lambda_S (\beta_{i} - \beta{j})^2$ can be incorporated into a least-squares }\DIFaddend regression problem by \DIFdelbegin \DIFdel{vectorization}\DIFdelend \DIFaddbegin \DIFadd{appending a row containing $\sqrt{lambda_S}$ in the $i$th position, $-\sqrt{lambda_S}$ in the $j$th position, and $0$s elsewhere to the design matrix, and zero to the response vector}\DIFaddend . 

\DIFdelbegin \DIFdel{Specifically, if $Y \in R^{N \times M}$ then we form $A =$ diagonally concatenate $X$, $M$ times }\DIFdelend \DIFaddbegin \DIFadd{In order to convert an optimization over multiple response variables (indexed by $i$) and multiple sources (indexed by $j$) with design matrices $X^{S_j}$ and response variables $y_i^{S_j}$ into an optimization with a single source and response variable, we vectorize as follows. Let $G$ be a set containing $(i,j)$ for each of the data sources and response variables currently being optimized. We construct the matrix $A$ by diagonally concatenating each of $X^{S_j}$ }\DIFaddend (with the remaining entries 0) \DIFdelbegin \DIFdel{,
$y =$ vertically concatenate the columns of $Y$. We solve 
}\DIFdelend \DIFaddbegin \DIFadd{and vertically concatenating each of $y_j^{S_j}$. We then solve
%DIF > Specifically, if $Y \in R^{N \times M}$ then we form $A =$ diagonally concatenate $X$, $M$ times (with the remaining entries 0),
%DIF > $y =$ vertically concatenate the columns of $Y$. We solve 
}\DIFaddend 

$$\underset{W}{min} ||AW - y||_2^{~2}$$

%We can then define $W_{i,j} = \beta_{(j-1) \times N + i, (j-1) \times M + j}$ which is the desired form.
This is equivalent to the original problem due to the block structure of matrix multiplication. In an ordinary \DIFdelbegin \DIFdel{ridge regression problem , this is unnecessary, as the columns of the weight matrix }\DIFdelend \DIFaddbegin \DIFadd{regression problem each response variable }\DIFaddend can be solved independently \DIFaddbegin \DIFadd{as a large number of small regression problems}\DIFaddend . However, in fused regression, \DIFdelbegin \DIFdel{information is shared between columns, and therefore they }\DIFdelend \DIFaddbegin \DIFadd{we append additional rows to the design matrix $A$ that link entries of $W$ associated with different response variables. As a result, these linked response variables }\DIFaddend must be solved \DIFdelbegin \DIFdel{for simultaneously . If two entries are in the same fusion constraint , or if there exists a constraint chain containing entries from two columns, those columns must be solved together. Two entries may be fused if say, two TFs are orthologous, and two genes are orthologous--then, the interaction between the first TF, gene pair should be similar to the second TF, gene pair. A constraint chain can exist if there are complex orthology mappings, say, }\DIFdelend \DIFaddbegin \DIFadd{simultaneously through vectorization. Two response variables are linked by a fusion constraint }\DIFaddend if \DIFdelbegin \DIFdel{there are multiple similar sequence signals, and three or more entries are fused. }%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < Given $X_1... X_S$ matrices, $X_i \in R^{N_i \times K_i}$ we define $N = \sum_{i=0}^S N_i$, $K = \sum_{i=0}^SK_i$ and $X \in R^{N \times K}$ by diagonally concatenating $X_1...X_S$.
%DIF < In this new matrix, the first $K_1$ features correspond to features of $X_1$, the next $K_2$ features the features of $X_2$ etc.
%DIF < Similarly, the first $N_1$ samples correspond to samples of $X_1$, the next $N_2$ to $X_2$ etc.
%DIF < Solving the regression problem associated with each source is equivalent to solving the problem $\underset{W}{argmin}||XW-Y||$ where $Y$ is defined in the same way as $X$.
%DIF < The weight matrix $W$ has a block diagonal structure:
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < \begin{figure}[h!]
%DIF <     \centering
%DIF <     \includegraphics[scale=.5,trim=0mm 0mm 0mm 0mm,clip]{direct_solve.png}
%DIF < \end{figure}
%DIFDELCMD < 
...
%DIFDELCMD < %%%
%DIF < The regression problems associated with two columns $A,A'$ of $Y$, where $A$ came from $Y_1$ and $A'$ came from $Y_2$, are independent, unless there is a constraint chain of length $L$ with the form
%DIFDELCMD < 

%DIFDELCMD < %%%
%DIF < $$(G, j_0, g, J_1, j_1, m_1) \rightarrow (J_1, j_1, m_1, J_2, j_2, m_2) \rightarrow (J_2,j_2,m_2,J_3,j_3,_m3)...(J_P, j_P, m_P, H, j_{P+1}, h)$$
%DIF < $$(Y_1, j_1, g, Y_2, j_2, h) \rightarrow (Y_2, j_2, h, Y_3, j_3, k)$$
\DIFdel{$((\beta_1, A, B), (\beta_1, A, C), (\beta_2, A', B'))$
}%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdel{Here, we have two datasets, which we refer to as 1 and 2. We have information about the similarity of TF A in network 1 to TF A' in network 2, and about the similarity of genes B and C in network 1 to gene B' in network 2. We expect, therefore, that if an interaction exists between A and B or A and C in the first network, that these interactions be similar to the interaction between A' and B' in the second network, and we penalize solutions which do not have this feature}\DIFdelend \DIFaddbegin \DIFadd{any of the regulatory weights affecting those genes are linked by a fusion constraint. Two response variables must be solved simultaneously if there is any chain of linked response variables connecting them. However, every other response variable can be solved separately. In biological terms, the regulators of two genes (whether in the same species, or different species) must be solved together if there is a fusion constraint linking those genes' regulators, or if there is a chain of such constraints. This is important because vectorization produces a design matrix with a number of rows and columns equal to the sum of the number of rows and columns of the design matrices associated with each of the response variables (genes) being solved for. If the networks for a large number of Genes are solved simultaneously, the system of equations can quickly become intractible to solve}\DIFaddend . 

\DIFdelbegin \DIFdel{To find all such linked problems}\DIFdelend \DIFaddbegin \DIFadd{In order to avoid this difficulty}\DIFaddend , we use depth-first search to identify linked columns of \DIFdelbegin \DIFdel{the }\DIFdelend \DIFaddbegin \DIFadd{each }\DIFaddend TF expression matrix \DIFdelbegin \DIFdel{$Y$, then solve the fused regression problem directly by vectorizing corresponding columns. To incorporate the fusion penalty, we append to the resulting block diagonal design matrix one row per connected constraint, which contains $\lambda_S$ in the column associated with the TF, gene pair (A, B) from dataset 1, and $-\lambda_S$ to the column associated with TF, gene pair (A', B') from dataset 2, as well as another one for (A, C), (A', B'). The vector $Y$ is augmented by appending an appropriate number of zeros. This results in the addition of the penalties, $\lambda_S||b_{A,B} - b_{A',B'}||^{2}$ and $\lambda_S||b_{A,C} - b_{A',B'}||^{2}$ to the squared error $||AW-y||^2$. }%DIFDELCMD < 

%DIFDELCMD < %%%
\DIFdelend \DIFaddbegin \DIFadd{$Y^{S_j}$, then forming design and response matrices through vectorization. We can then incorporate fusion constraints as in the case of single-source single response-variable fused regression. }\DIFaddend In most cases, we have found the direct solve using augmented matrices to be adequate. \DIFdelbegin \DIFdel{However, there are certainly use cases of fused regression where solving the fused regression problem directly, even after factoring constraints, involves at least one inversion of an $p\sum_{i=0}^S K_i \times p \sum_{i=0}^S K_i$ matrix where $K_i$ is }\DIFdelend \DIFaddbegin \DIFadd{This is because the structure of orthology tends to be sparse, so only a small number of genes must be solved at once. However, in }\DIFaddend the \DIFdelbegin \DIFdel{number }\DIFdelend \DIFaddbegin \DIFadd{general case of $c_i$ response variables from each }\DIFaddend of \DIFdelbegin \DIFdel{features in each network and $p$ is the size of a column subset such that there is a constraint chain linking an element of each column to an element in every other column in that subset, taking $O((p\sum_{i=0}^S K_i)^3)$}\DIFdelend \DIFaddbegin \DIFadd{$i \in \text{species}$ data sources the design matrix to be solved is of size $\displaystyle \sum_{c_i} rows(X^{S_i}) \times \displaystyle \sum_{c_i} cols(X^{S_i})$}\DIFaddend . Although the sparse structure of \DIFdelbegin \DIFdel{$X_P$ }\DIFdelend \DIFaddbegin \DIFadd{the augmented design matrix }\DIFaddend makes this somewhat more tractable, it is still impractical for large \DIFaddbegin \DIFadd{constraint }\DIFaddend chains.
...
Our adaptive penalty function introduces, in addition to regularization and fusion penalty weights $\lambda_R$ and $\lambda_S$, an unknown parameter $a$. We could employ grid search using cross-validation to searach for the best parameters, but for many data sets, this can be computationally expensive. Moreover, we are primarily interested in using this saturating penalty as a way of testing the hypothesis that conservation in GRNs can be predicted based off of known similarities between genes. Therefore, we implement a way of \DIFdelbegin \DIFdel{picking }\DIFdelend \DIFaddbegin \DIFadd{selecting }\DIFaddend $a$ \DIFdelbegin \DIFdel{with a user-selected percentile of differences in interaction weights, which }\DIFdelend \DIFaddbegin \DIFadd{to be equal to a certain percentile of the distribution of differences in fused interaction weights from the networks fit without fusion. This choice }\DIFaddend represents the user's hypothesis for the \DIFdelbegin \DIFdel{proportion of evolutionary constraints which should be relaxed}\DIFdelend \DIFaddbegin \DIFadd{fraction of both networks that have unrelated function, and should therefore be unfused}\DIFaddend . 
...
\end{equation}
When the residual is uncorrelated with the prediction, then removing regressor $i$ with weight $B_{i,j}^2$ from the model of $j$ will increase the residual by $B_{i,j}^2$ times the variance of regressor $i$. In the case of regularized regression, the regressor and residual need not be uncorrelated, so the approximation will not hold exactly. This scaling is similar to rescaling according to variance explained relative to an augmented design matrix that includes fusion constraints. \DIFdelbegin \DIFdel{We use an approximation because it is simpler to compute. The intuition is }\DIFdelend \DIFaddbegin \DIFadd{The intuition for scaling }\DIFaddend that a large $B_{i,j}$ may be \DIFdelbegin \DIFdel{necessary because }\DIFdelend \DIFaddbegin \DIFadd{inferred because of overfitting with a }\DIFaddend TF $i$ varies little across the available data, \DIFdelbegin \DIFdel{and these large weights are not }\DIFdelend \DIFaddbegin \DIFadd{or because the regulatory weight is strong. Only in the latter case is a large weight }\DIFaddend suggestive of a true regulatory interaction.
